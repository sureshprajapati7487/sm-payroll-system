...
export const WorkGroupManager = ({ onClose }: Props) => {
    ...
    const departments = useMemo(() => {
        ...
        activeEmployees.forEach(e => { if (e.department) s.add(e.department); });
    ...
    const handleAdd = () => {
        ...
        addGroup({ name: newName.trim(), department: newDept || 'General', color: 'blue', icon: '' });
        setNewName('');
    };
    ...
    const getEmpOptions = (group: WorkGroup) =>
        activeEmployees.filter(e =>
            ...
            (e.name.toLowerCase().includes(empSearch.toLowerCase()) || e.code.toLowerCase().includes(empSearch.toLowerCase()))
        );
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-72 border-r border-dark-border flex flex-col bg-dark-bg/20 shrink-0">
                        ...
                        <div className="p-4 border-b border-dark-border bg-dark-bg/30">
                            <p className="text-[10px] font-bold text-dark-muted uppercase tracking-wider mb-3">Add Department</p>
                        ...
                        <div className="flex-1 overflow-auto p-2 space-y-2">
                            {departments.map(dept => {
                                ...
                                return (
                                    <div key={dept}>
                                        <p className="text-[9px] font-bold text-dark-muted uppercase tracking-widest px-2 py-1">{dept}</p>
                                        {deptGroups.map(g => {
                                            const col = GROUP_COLOR_MAP[g.color] || GROUP_COLOR_MAP.blue;
                                            const members = getGroupMembers(g);
                                            const isActive = assigningGroupId === g.id;
                                            return (
                                                <div key={g.id} onClick={() => setAssigningGroupId(isActive ? null : g.id)}
                                                    className={clsx(
                                                        'flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer transition-all mb-1 group',
                                                        isActive ? `${col.bg} ${col.border} border` : 'hover:bg-dark-bg/50'
                                                    )}>
                                                    <div className={clsx('w-2 h-2 rounded-full', col.bg, col.border, 'border')} />
                                                    <div className="flex-1 min-w-0">
                                                        <p className={clsx('text-sm font-medium truncate', isActive ? col.text : 'text-white')}>{g.name}</p>
                                                        <p className="text-[10px] text-dark-muted">{members.length} employees</p>
                                                    </div>
                                                    {/* Delete */}
                                                    <button onClick={e => { e.stopPropagation(); if (window.confirm(`"${g.name}" delete karna chahte ho?`)) removeGroup(g.id); }}
                            ...
                            {Array.from(groupsByDept.entries())
                                ...
                                .map(([dept, deptGroups]) => (
                                    <div key={dept}>
                                        <p className="text-[9px] font-bold text-dark-muted uppercase tracking-widest px-2 py-1">{dept || 'General'}</p>
                                        {deptGroups.map(g => {
                                            const members = getGroupMembers(g);
                                            const isActive = assigningGroupId === g.id;
                                            return (
                                                <div key={g.id} onClick={() => setAssigningGroupId(isActive ? null : g.id)}
                                                    className={clsx(
                                                        'flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer transition-all mb-1 group',
                                                        isActive ? 'bg-violet-500/10 border border-violet-500/30' : 'hover:bg-dark-bg/50'
                                                    )}>
                                                    <div className="w-2 h-2 rounded-full bg-violet-500/30 border border-violet-500" />
                                                    <div className="flex-1 min-w-0">
                                                        <p className={clsx('text-sm font-medium truncate', isActive ? 'text-violet-400' : 'text-white')}>{g.name}</p>
                                                        <p className="text-[10px] text-dark-muted">{members.length} employees</p>
                                                    </div>
                                                    <button onClick={e => { e.stopPropagation(); if (window.confirm(`"${g.name}" delete karein?`)) removeGroup(g.id); }}
                                                        className="opacity-0 group-hover:opacity-100 text-dark-muted hover:text-danger p-1 rounded transition-all">
                                                        <Trash2 className="w-3.5 h-3.5" />
                                                    </button>
                                                </div>
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        {assigningGroupId ? (() => {
                            ...
                            const col = GROUP_COLOR_MAP[group.color] || GROUP_COLOR_MAP.blue;
                            const members = getGroupMembers(group);
                            const options = getEmpOptions(group);

                            return (
                                <>
                                    <div className={clsx('px-5 py-3 border-b border-dark-border flex items-center gap-3', col.bg)}>
                                        <div>
                                            <h3 className={clsx('font-bold text-base', col.text)}>{group.name}</h3>
                                            <p className="text-[10px] text-dark-muted">{group.department} · {members.length} assigned</p>
                                        </div>
                                        <div className="ml-auto">
                                            <input type="text" placeholder="Search employee..."
                                                value={empSearch} onChange={e => setEmpSearch(e.target.value)}
                                                className="bg-dark-bg border border-dark-border rounded-lg px-3 py-1.5 text-white text-xs focus:outline-none focus:border-primary-500 w-44" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-auto p-4 space-y-4">
                                        ...
                                        {members.length > 0 && (
                                            <div>
                                                <p className="text-[10px] font-bold text-dark-muted uppercase tracking-wider mb-2 flex items-center gap-1">
                                                    <Users className="w-3 h-3" /> Assigned ({members.length})
                                                </p>
                                                <div className="space-y-1">
                                                    {members.map(emp => (
                                                        <div key={emp.id}
                                                            className={clsx('flex items-center gap-3 px-3 py-2 rounded-lg', col.bg)}>
                                                            <div className={clsx('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold', col.badge)}>
                                                                {emp.name[0]}
                                                            </div>
                                                            <div className="flex-1">
                                                                <p className="text-sm text-white font-medium">{emp.name}</p>
                                                                <p className="text-[10px] text-dark-muted">{emp.code} · {emp.designation || '—'}</p>
                                                            </div>
                                                            <button onClick={() => unassignEmployee(emp.id)}
                                        ...
                                        {options.length > 0 && (
                                            <div>
                                                <p className="text-[10px] font-bold text-dark-muted uppercase tracking-wider mb-2 flex items-center gap-1">
                                                    <UserPlus className="w-3 h-3" /> Add from {group.department} ({options.length})
                                                </p>
                                                <div className="space-y-1">
                                                    {options.map(emp => (
                                                        <div key={emp.id}
                                                            ...
                                                            <button onClick={() => assignEmployee(emp.id, group.id)}
                                        ...
                                        {members.length === 0 && options.length === 0 && (
                                            <div className="text-center text-dark-muted opacity-50 py-12">
                                                ...
                                                <p className="text-sm">Is department mein aur employees nahi</p>
                                            </div>
                                        )}
                                    </div>
                                </>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-dark-muted opacity-40">
                                ...
                                <p className="text-sm font-medium">Left se koi department select karo</p>
                                <p className="text-xs mt-1">Employees assign/remove karne ke liye</p>
                            </div>
                        )}
                    </div>
                </div>
